                    ...
                    <div class="card card-bordered card-compact bg-base-100 shadow-xl">
                        ...
                        <div class="card-body">
                            ...
                            <div class="justify-center card-actions">
                                <a href="{{ route('kendaraan.edit', $kendaraan->id) }}" class="btn btn-neutral hover:btn-warning transition duration-300 ease-in-out transform hover:-translate-y-1">Edit</a>
                                <form action="{{ route('kendaraan.destroy', $kendaraan->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this Kendaraan?')" class="btn btn-neutral hover:btn-error transition duration-300 ease-in-out transform hover:-translate-y-1">Delete</button>
                                </form>
                            </div>
                        </div>
                    </div>
